"""
Initial collection of 15-minute OHLCV bars for all NSE equities and macro instruments.
...
Usage:
  julia --project=packages/StockSwingPredictor scripts/collect_15min_ohlcv.jl
  julia --project=packages/StockSwingPredictor scripts/collect_15min_ohlcv.jl --equities-only
  julia --project=packages/StockSwingPredictor scripts/collect_15min_ohlcv.jl --macro-only
  julia --project=packages/StockSwingPredictor scripts/collect_15min_ohlcv.jl --refresh
  julia --project=packages/StockSwingPredictor scripts/collect_15min_ohlcv.jl --symbol RELIANCE
"""
...
function parse_args()
    ...
    while i <= length(ARGS)
        ...
        if a in ("-h", "--help")
            println("""
collect_15min_ohlcv.jl — initial 15-minute OHLCV collection (last 199 days)

Flags:
  --equities-only   Only fetch equity 15-min (skip macro)
  --macro-only      Only fetch macro 15-min (skip equities)
...
function main()
    ...
    # 200-day retention; fetch from 199 days ago to yesterday
    from_date = today() - Day(199)
    to_date   = today() - Day(1)

    @info "15-min collection: $from_date → $to_date"
    ...
    if !args["macro_only"]
        ...
        @info "── Equity 15-min: $(length(symbols)) symbols ──"
        instr     = load_instruments(session; refresh=true)
        token_map = build_token_map(instr)

        collect_ohlcv_15min(symbols, token_map, session, OHLCV_DIR,
                             from_date, to_date; refresh=refresh)
    end

    # ── Macro ─────────────────────────────────────────────────────────────────
    if !args["equities_only"]
        @info "── Macro 15-min: $(length(KITE_MACRO_INSTRUMENTS)) instruments ──"
        collect_macro_15min(session; from_date=from_date, to_date=to_date,
                            refresh=refresh)
    end
end
