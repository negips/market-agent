...
Only companies that have already been confidence-scored (by run_confidence_checks.jl)
AND have a confidence score ≥ 40 are included.
...
""")
    ...
    # Build symbol → company dict (keep only confidence-scored + passing companies)
    company_by_symbol = Dict{String, Any}()
    for c in all_c
        conf = get(c, :confidence, nothing)
        isnothing(conf) && continue
        score = get(conf, :score, 0)
        score >= 40     || continue
    ...
    @info "Loaded $(length(all_c)) companies, $n_eligible with confidence ≥ 40"
